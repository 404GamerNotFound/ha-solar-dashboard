...
- `entities.electricity_price` (entity id, optional; current import price, for example from Tibber or aWATTar. A numeric state overrides `grid_import_price`; values in `ct/kWh`, `€/kWh`, `€/MWh`, and equivalent common units are normalized to currency per kWh. The current tariff is shown on the grid tile, and the daily cost is explicitly labelled as an estimate at that rate.)
